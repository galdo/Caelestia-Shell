...
Singleton {
    ...
    onCapsLockChanged: {
        ...
        if (capsLock)
            Toaster.toast(Tr.t("Caps lock enabled"), Tr.t("Caps lock is currently enabled"), "keyboard_capslock_badge");
        else
            Toaster.toast(Tr.t("Caps lock disabled"), Tr.t("Caps lock is currently disabled"), "keyboard_capslock");
    }
    ...
    onNumLockChanged: {
        ...
        if (numLock)
            Toaster.toast(Tr.t("Num lock enabled"), Tr.t("Num lock is currently enabled"), "looks_one");
        else
            Toaster.toast(Tr.t("Num lock disabled"), Tr.t("Num lock is currently disabled"), "timer_1");
    }

    onKbLayoutFullChanged: {
        if (hadKeyboard && GlobalConfig.utilities.toasts.kbLayoutChanged)
            Toaster.toast(Tr.t("Keyboard layout changed"), Tr.t("Layout changed to: %1").arg(kbLayoutFull), "keyboard");
